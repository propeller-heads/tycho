...
2. List your executor in <a href="https://github.com/propeller-heads/tycho-indexer/blob/main/crates/tycho-execution/contracts/test/RuntimeBytecodeFixtures.t.sol" target="_blank" rel="noopener noreferrer">`crates/tycho-execution/contracts/test/RuntimeBytecodeFixtures.t.sol`</a>, pointing at the same `(chain, protocol)` entry. Give it a fork of its own when its constructor reads state the default fork does not carry — an address it checks for code, or a value it keeps in an immutable.
3. Regenerate the fixtures from `crates/tycho-execution/contracts` with `FIXTURES_WRITE=1 forge test --match-contract RuntimeBytecodeFixtures`, then commit the `YourExecutor.runtime.json` it writes to <a href="https://github.com/propeller-heads/tycho-indexer/tree/main/protocols/testing/fixtures" target="_blank" rel="noopener noreferrer">`protocols/testing/fixtures/`</a>. `forge test` checks the committed fixtures against the contracts in CI, so a fixture this test does not list goes stale unnoticed.
