...
<tr><td><code>vm:tessera</code></td><td>VM (<code>EVMPoolState</code>)</td><td>-</td><td>Base</td><td>Wintermute's propAMM. One component per base/USDC book; a delisted or stale book quotes zero and drops out via limits. Majors (WETH, cbBTC, EURC) are recommended for routing; see the integration notes on quote drift for the long tail.</td></tr>
    ...
    match chain {
        Chain::Ethereum => {
            builder = builder
                ...
                .exchange::<EVMPoolState<PreCachedDB>>("vm:tessera", tvl_filter.clone(), None)
        }
        Chain::Base => {
            builder = builder
                .exchange::<UniswapV2State>("uniswap_v2", tvl_filter.clone(), None)
