...
<tr><td><code>uniswap_v4_hooks</code></td><td>Hybrid (<code>UniswapV4State</code>)<br>[DCI indexed]</td><td>1 ms</td><td>Ethereum, Robinhood</td><td>Ethereum simulates every composable hook.<br><strong>Angstrom</strong>: see more details <a href="supported-protocols.md#angstrom-uniswap-v4-hook">below</a>.<br>Robinhood simulates only pools using the Pons V2 MemeHook (<code>0xe5e702641ea86f4ae6cc3cdaed2b886f976be044</code>), natively; pools using any other hook fail to decode there, since Robinhood carries no generic VM fallback.<br><strong>Pons V2</strong>: see more details <a href="supported-protocols.md#pons-v2-uniswap-v4-hook-robinhood">below</a>.<br><em>recommended</em>: set a high startup timeout on the stream builder: <code>.startup_timeout(Duration::from_secs(120))</code></td></tr>
...
#### Pons V2 (Uniswap V4 Hook, Robinhood) <a href="#pons-v2-uniswap-v4-hook-robinhood" id="pons-v2-uniswap-v4-hook-robinhood"></a>

Pons V2's `V2MemeHook` runs an `afterSwap` callback that charges the swap's output currency: it takes `hookFeeBps` plus `creatorTaxBps` of the output, flooring each rate independently before summing the two terms. `registerPool` fixes both rates once, for the life of the pool, and sets the core pool's LP fee to zero, so the hook's cut stands as the only fee a Pons pool charges.

The indexer emits three static attributes for a Pons pool: `hook_identifier` (`pons_v2`), `pons_hook_fee_bps`, and `pons_creator_tax_bps`. `spot_price` marks up the pool's core price by the hook's rate, so a quoted buy price already carries the fee. `get_limits` reports the net amount a swapper receives, after the hook takes its cut. Tycho quotes exact-input swaps only for Pons pools; it does not support exact-output for them. Neither `hookData` nor the swap sender changes the fee, since `registerPool` fixes both rates once and nothing else in the callback reads them.
