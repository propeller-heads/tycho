...
pub struct Config {
    ...
    /// The `PrioUpdateRegistry` the router reads quote lanes from.
    ///
    /// The router switched registry at block 25989123. This is the one it reads now; the previous
    /// instance is no longer written to and decoding `updateState` on it would pin simulation to a
    /// lane the router never consults. The switch is a plain storage write and the router emits no
    /// events, so it cannot be followed the way `VaultUpdated` is followed for the vault -- a
    /// further migration needs this parameter changed.
    #[serde(with = "hex::serde")]
    ...
    fn test_config_parses_substreams_params() {
        let config: Config = serde_qs::from_str(
            "router_address=00000003f1ec2379e79f58e12ec6c4f51ee92149\
             ...
             &registry_address=da7afeed021eafc1c1af9c362de477dad0396b81",
        )
        .unwrap();
        ...
        assert_eq!(config.registry_address, addr("da7afeed021eafc1c1af9c362de477dad0396b81"));
    }
}
