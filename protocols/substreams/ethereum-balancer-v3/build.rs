...
fn main() -> Result<(), anyhow::Error> {
    ...
    for f in file_names {
        ...
        // QuantAMM create params are a 16-field tuple; Rust only implements Debug/PartialEq
        // for tuples up to 12 elements, so strip those derives from the generated file.
        if file_output_names[i].contains("quantamm_weighted_pool_factory_contract") {
            ...
            let fixed = generated.replace("#[derive(Debug, Clone, PartialEq)]", "#[derive(Clone)]");
            fs::write(file_output_names[i], fixed)?;
        }
