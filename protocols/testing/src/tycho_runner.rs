...
impl TychoRunner {
    ...
    pub fn run_tycho(
        ...
        cmd.args([
            ...
            "--stop-block",
            // The substreams stop block is exclusive, and the indexer commits every buffered
            // block once the stream ends, so the database holds the state at exactly `end_block`.
            &(end_block + 1).to_string(),
            "--dci-plugin",
            "rpc",
        ]);
