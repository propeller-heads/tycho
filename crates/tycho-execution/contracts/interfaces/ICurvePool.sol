...
/// @notice The Curve pool type byte the encoder emits.
library CurvePoolType {
    /// @notice Stable (1) and stable_ng (10) pools take the `int128` index signatures; crypto
    /// and llamma pools take `uint256`.
    function isStable(uint8 poolType) internal pure returns (bool) {
