...
- **`rfq/`**: the book feeds that need binding quotes at execution time (Bebop, Hashflow,
  ...
  quotes through `IndicativelyPriced`; the crate-private `RFQError` covers the quoting layer and
  never reaches a public signature. Credentials are constructor arguments of the feed builders; the
