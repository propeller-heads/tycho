...
impl FailureTracker {
    pub(crate) fn new(max: Option<u32>) -> Self {
    ...
    pub(crate) fn backoff(&self, unit: Duration, max_exp: u32) -> Duration {
...
pub(crate) trait HttpBookSource: Send + Sync {
...
/// reference.
pub(crate) trait WsBookSource: Send {
...
pub(crate) async fn run_http_poll_feed(
    provider: &str,
...
#[instrument(name = "book_feed", skip(config, tx, source))]
pub(crate) async fn run_ws_feed(
    provider: &str,
    config: WsFeedConfig,
    tx: watch::Sender<Option<BookSnapshot<ReceivedAt>>>,
...
mod tests {
    ...
    use crate::{
        ...
        rfq::protocols::bebop::{client::BebopClient, models::BebopBook, state::BebopState},
    };

    /// One book with a single empty pair, enough for the loops to publish something.
    fn book() -> HashMap<String, Book> {
        ...
        let book = BebopBook {
            base: token.address.clone(),
            quote: token.address.clone(),
            last_update_ts: 0,
            bids: Levels::default(),
            asks: Levels::default(),
        };
    ...
    mod http_poll_feed {
        ...
        async fn withdraws_the_published_book_when_it_gives_up() {
            ...
            timeout(Duration::from_secs(1), async {
                loop {
                    rx.changed().await.expect("feed ended before publishing");
                    if rx.borrow_and_update().is_some() {
                        return;
                    }
                }
