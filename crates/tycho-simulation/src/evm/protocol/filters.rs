...
/// is not indexed, so the pool cannot be simulated. The Substreams package records the address
/// of the pool's last `Incentive` event in the `active_incentive` state attribute; a pool is
/// usable while that attribute is absent or the zero address.
