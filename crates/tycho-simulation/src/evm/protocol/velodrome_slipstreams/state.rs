...
impl Eq for VelodromeSlipstreamsState {}
...
impl VelodromeSlipstreamsState {
    ...
    fn swap_core(
        ...
        amount_in: U256,
        zero_for_one: bool,
    ) -> Result<(U256, U256, VelodromeSlipstreamsState), SimulationError> {
...
#[typetag::serde]
impl SwapQuoter for VelodromeSlipstreamsState {
    fn component(&self) -> SimulationResult<Arc<ProtocolComponent<Arc<Token>>>> {
        self.component.clone().ok_or_else(|| {
            SimulationError::FatalError(
                "VelodromeSlipstreamsState: component not set (decode did not populate it)"
                    .to_string(),
            )
        })
    }
    ...
    fn marginal_price(&self, params: MarginalPriceParams) -> SimulationResult<MarginalPrice> {
        let component = self.component.as_ref().ok_or_else(|| {
            SimulationError::FatalError("VelodromeSlipstreamsState: component not set".to_string())
        })?;
        let base = component
        ...
        let amount_in =
            match params.amount() {
                ...
                QuoteAmount::FixedOut(_) => return Err(SimulationError::RecoverableError(
                    "VelodromeSlipstreamsState does not yet support exact-out (FixedOut) quoting"
                        .to_string(),
                )),
            };
    ...
    fn query_swap(&self, _params: QuerySwapParams) -> SimulationResult<Swap> {
        Err(SimulationError::FatalError(
            "VelodromeSlipstreamsState::query_swap is not yet wired (pending token plumbing)"
                .to_string(),
        ))
    }
...
mod tests {
    ...
    fn test_marginal_price_matches_spot_price() {
        ...
        dto.tokens = vec![t0.address.clone(), t1.address.clone()];
        let all_tokens =
            HashMap::from([(t0.address.clone(), t0.clone()), (t1.address.clone(), t1.clone())]);
        let component =
            crate::evm::protocol::build_swap_quoter_component(&dto, &all_tokens).unwrap();
        let state = pool.with_component(component);

        for (base, quote) in [(&t0, &t1), (&t1, &t0)] {
