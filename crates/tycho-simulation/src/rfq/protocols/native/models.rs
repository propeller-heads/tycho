...
/// One side of a book: the ladder a fill walks, and the trade sizes the side requires. A side
/// comes from a single orderbook entry, which states its minimum on one end of the trade — the
/// input for a bid, the output for an ask — so the end it does not constrain stays `0.0`.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct NativeBookSide {
    pub levels: Levels,
    /// Minimum a taker must sell into this side, in atomic units of the token it takes in.
    pub minimum_in: f64,
    /// Minimum a taker must receive from this side, in atomic units of the token it pays out.
    pub minimum_out: f64,
}
...
pub struct NativePriceData {
    pub base_address: Bytes,
    pub quote_address: Bytes,
    /// The makers' bids: a taker fills them by selling base and receiving quote.
    pub bids: NativeBookSide,
    /// The makers' asks: a taker fills them by selling quote and receiving base.
    pub asks: NativeBookSide,
}
...
impl NativePriceData {
    ...
    pub fn calculate_tvl(&self, quote_price_data: Option<&NativePriceData>) -> Option<f64> {
        let bid_tvl = self.bids.levels.notional();
        let ask_tvl = self.asks.levels.notional();
        let total_tvl = match (self.bids.levels.is_empty(), self.asks.levels.is_empty()) {
    ...
    pub fn get_mid_price(&self, amount: f64, sell_token: &Bytes) -> Option<f64> {
        ...
        let (bids, asks) = (&self.bids.levels, &self.asks.levels);
        let (bids_price, asks_price) = if sell_token == &self.quote_address {
            (bids.invert().average_price(amount), asks.invert().average_price(amount))
        } else {
            (bids.average_price(amount), asks.average_price(amount))
        };
...
mod tests {
    ...
    fn normalizes_tvl_through_quote_token_market() {
        ...
        let price_data_eth_tamara = NativePriceData {
            ...
            bids: side(vec![PriceLevel { quantity: 3.0, price: 100.0 }]),
            asks: side(vec![PriceLevel { quantity: 3.0, price: 100.0 }]),
        };
        let price_data_tamara_usdc = NativePriceData {
            base_address: tamara,
            quote_address: usdc,
            bids: side(vec![PriceLevel { quantity: 300.0, price: 9.0 }]),
            asks: side(vec![PriceLevel { quantity: 300.0, price: 11.0 }]),
        };
    ...
    fn calculates_and_normalizes_tvl_for_one_sided_bid_books() {
        ...
        let price_data_tamara_usdc = NativePriceData {
            base_address: tamara,
            quote_address: addr("0xA0b86991c6218b36c1d19d4a2e9Eb0cE3606eB48"),
            bids: side(vec![PriceLevel { quantity: 300.0, price: 10.0 }]),
            asks: NativeBookSide::default(),
        };
    ...
    fn rejects_non_finite_derived_tvl() {
        ...
        let book = |base_address, quote_address, quantity, price| NativePriceData {
            base_address,
            quote_address,
            bids: side(vec![PriceLevel { quantity, price }]),
            asks: NativeBookSide::default(),
        };
