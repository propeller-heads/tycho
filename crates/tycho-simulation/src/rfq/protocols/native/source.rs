...
impl NativeBookSource {
    ...
    fn build_book(
        ...
        quote_token: Token,
        book: NativePriceData,
    ) -> Result<(ProtocolComponent, NativeState), FeedError> {
        ...
        .map_err(|e| FeedError::ParsingError(format!("Invalid Native book: {e}")))?;
...
impl HttpBookSource for NativeBookSource {
    ...
    async fn fetch_books(&self) -> Result<HashMap<String, Book>, FeedError> {
        ...
        for (component_id, book) in grouped {
            ...
            let (component, state) =
                match self.build_book(&component_id, base_token.clone(), quote_token.clone(), book)
                {
                    Ok(pair) => pair,
                    Err(e) => {
                        warn!(%component_id, error = %e, "skipping pair, invalid book");
                        continue;
                    }
                };
...
mod tests {
    ...
    fn builds_component_and_state_from_relay_orderbook() {
        ...
        let (component, state) = source
            .build_book(&component_id, base_token.clone(), quote_token.clone(), book.clone())
            .unwrap();
        ...
        let expected =
            NativeState::new(base_token, quote_token, book, Arc::clone(&source.client)).unwrap();
