...
// `Deserialize` bypasses `new`, so it does not check the book's addresses and minimums (the
// levels validate themselves). This is harmless while `delta_transition` cannot update the book;
// use `TryFrom`-based deserialization before supporting state deltas.
...
impl NativeState {
    /// Fails when the book's addresses do not match the tokens or a minimum is not a finite
    /// non-negative number.
    pub(crate) fn new(
        base_token: Token,
        quote_token: Token,
