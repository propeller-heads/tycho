...
use crate::{
    book::{
        errors::FeedError,
        feed_loops::run_ws_feed,
        models::{default_ws_feed_config, BookSnapshot, CommonConfig, ReceivedAt, WsFeedConfig},
    },
    rfq::{
        constants::DEFAULT_QUOTE_TIMEOUT,
        protocols::bebop::{client::BebopClient, source::BebopBookSource, PROTOCOL_SYSTEM},
    },
...
fn chain_to_bebop_url(chain: Chain) -> Result<String, FeedError> {
    let chain_path = match chain {
        ...
        Chain::Base => "base",
        _ => return Err(FeedError::FatalError(format!("Unsupported chain: {chain:?}"))),
    };
...
/// Builds a [`BebopFeed`] from the shared [`CommonConfig`] and Bebop's API key.
///
...
///     book::{constants::usd_stablecoins_for_chain, models::CommonConfig},
///     rfq::protocols::bebop::feed::BebopFeedBuilder,
/// };
...
/// let feed = BebopFeedBuilder::new(common, usd_quote_tokens, "ws_key".to_string())
///     .build()
///     .unwrap();
/// ```
    ...
    pub fn build(self) -> Result<BebopFeed, FeedError> {
        ...
        Ok(BebopFeed {
            ...
            source: BebopBookSource {
                ...
                price_ws: format!("wss://{url}/pricing?format=protobuf"),
...
impl SnapshotFeed for BebopFeed {
    ...
    fn subscribe(
        ...
        let BebopFeed { feed_config, source } = self;
        let feed = run_ws_feed(PROTOCOL_SYSTEM, feed_config, tx, source);
        (rx, feed)
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::book::models::{test_token_map, Book};
