...
use crate::{
    book::{
        component::{directed_pair_component_id, pair_component},
        errors::FeedError,
...
pub struct HashflowBookSource {
    ...
    pub price_levels_endpoint: String,
    pub market_makers_endpoint: String,
    /// Requests binding quotes; shared with every state this source emits.
    pub client: Arc<HashflowClient>,
}

impl HashflowBookSource {
    /// The TVL in USD quote-token units: `raw_tvl` as is when `quote_token` is one, otherwise
    /// converted through a level set that prices `quote_token` in one. `None` when the response
    /// carries no such level set.
    fn normalize_tvl(
        &self,
        raw_tvl: f64,
        quote_token: Bytes,
        ...
        // for normalization.
        for approved_quote_token in self.usd_quote_tokens.iter() {
            for mm_levels_inner in levels_by_mm.values() {
                for quote_mm_level in mm_levels_inner {
                    ...
                    {
                        if let Some(price) = quote_mm_level.levels.average_price(1.0) {
                            return Some(raw_tvl * price);
                        }
                    }
                }
            }
    ...
    fn chain_query(&self) -> Vec<(&'static str, String)> {
        vec![
            ("source", self.client.source().to_string()),
            ("baseChainType", "evm".to_string()),
            ("baseChainId", self.common.chain.id().to_string()),
        ]
    }

    async fn fetch_market_makers(&self) -> Result<Vec<String>, FeedError> {
        let request = self
            .client
            .http()
            ...
            .header("Authorization", self.client.auth_key());
    ...
    async fn fetch_price_levels(
        ...
        price_response
            .levels
            .ok_or_else(|| FeedError::ParsingError("API response missing levels".to_string()))
    }
}

impl HttpBookSource for HashflowBookSource {
    ...
    async fn fetch_books(&self) -> Result<HashMap<String, Book>, FeedError> {
        let market_makers = self.fetch_market_makers().await?;
        ...
        for (mm_name, mm_levels) in levels_by_mm.iter() {
            for mm_level in mm_levels {
                ...
                let component_id = directed_pair_component_id("hashflow", base_bytes, quote_bytes);
                let tvl = mm_level.levels.notional();
                let Some(normalized_tvl) =
                    self.normalize_tvl(tvl, mm_level.pair.quote_token.clone(), &levels_by_mm)
                else {
                    debug!(
                        %component_id,
                        market_maker = %mm_name,
                ...
                books.insert(
                    component_id,
                    Book { component, state: Arc::new(state), updated_at: None },
                );
            }
        }
        Ok(books)
...
mod tests {
    ...
    use crate::{
        book::{
            http::test_support::spawn_http_server,
            levels::{Levels, PriceLevel},
            models::test_token_map,
        },
    ...
    fn source_for(endpoint: &str, tokens: &[&Bytes], min_tvl_usd: f64) -> HashflowBookSource {
        ...
        HashflowBookSource {
            ...
            price_levels_endpoint: format!("{endpoint}/price-levels"),
            market_makers_endpoint: format!("{endpoint}/market-makers"),
    ...
    async fn fetch_books_builds_one_directed_book_per_pair() {
        ...
        let weth_usdc = directed_pair_component_id("hashflow", &weth, &usdc);
        let wbtc_weth = directed_pair_component_id("hashflow", &wbtc, &weth);
        let wbtc_usdc = directed_pair_component_id("hashflow", &wbtc, &usdc);
        assert_eq!(
            books
                .keys()
                .cloned()
                .collect::<HashSet<_>>(),
            HashSet::from([weth_usdc.clone(), wbtc_weth.clone()]),
            "below-floor {wbtc_usdc} must be absent"
        );
        let book = &books[&wbtc_weth];
        assert_eq!(book.updated_at, None);
        assert_eq!(book.component.id, Bytes::from(wbtc_weth.as_str()));
        let state = book
            .state
            .as_any()
    ...
    /// Hashflow signals failure inside an HTTP 200: a non-success status is an invalid-input
    /// error carrying the API's message, a success without levels a parsing error.
    #[rstest]
    #[case::status_fail_with_message(
        r#"{"status":"fail","error":"rate limited"}"#,
        FeedError::InvalidInput("rate limited".to_string())
    )]
    #[case::success_without_levels(
        r#"{"status":"success"}"#,
        FeedError::ParsingError(String::new())
    )]
    #[tokio::test]
    async fn price_levels_status_fail_and_missing_levels_are_errors(
        ...
        match (&error, &expected) {
            (FeedError::InvalidInput(msg), FeedError::InvalidInput(text)) => {
                assert!(msg.contains(text), "{msg}")
            }
            (FeedError::ParsingError(_), FeedError::ParsingError(_)) => {}
