...
use crate::{
    ...
    rfq::protocols::liquorice::{
        ...
        models::{LiquoricePriceLevelsResponse, LiquoriceTokenPairPrice},
...
impl LiquoriceBookSource {
    ...
    /// converted through a pair that prices `quote_token` in one. `None` when the response
        ...
        for approved_quote_token in self.usd_quote_tokens.iter() {
            for token_prices in prices_by_mm.values() {
                for token_price in token_prices {
                    ...
                    {
                        if let Some(price) = token_price.levels.average_price(1.0) {
    ...
    async fn fetch_price_levels(
        ...
        let request = self
            ...
            .query(&[("chainId", self.common.chain.id().to_string())])
            .header("accept", "application/json")
            .header("solver", self.client.auth_solver())
...
impl HttpBookSource for LiquoriceBookSource {
    async fn fetch_books(&self) -> Result<HashMap<String, Book>, FeedError> {
        let prices_by_mm = self.fetch_price_levels().await?;
        ...
        for (mm_name, token_pair_prices) in prices_by_mm.iter() {
            for token_pair_price in token_pair_prices {
                ...
                let Some(normalized_tvl) =
                    self.normalize_tvl(tvl, token_pair_price.quote_token.clone(), &prices_by_mm)
                else {
                    debug!(
                        market_maker = %mm_name,
                        base = %hex::encode(base_token),
                        quote = %hex::encode(quote_token),
                        "skipping pair, no levels price its quote token in a USD quote token"
                    );
                    continue;
                };
                if !self.common.clears_min_tvl(
                    normalized_tvl,
                    &format!(
                        "MM {mm_name} for pair {}/{}",
        ...
        for ((base_bytes, quote_bytes), mm_prices) in pair_mm_prices {
            ...
            let component_id = directed_pair_component_id("liquorice", &base_bytes, &quote_bytes);
...
mod tests {
    ...
    async fn fetch_books_groups_makers_per_pair_and_stamps_newest_update() {
        ...
        let component_id = directed_pair_component_id("liquorice", &weth, &usdc);
        assert_eq!(books.len(), 1, "one book for the one pair: {:?}", books.keys());
        let book = &books[&component_id];
        ...
        let expected = LiquoriceState::new(
            source.common.tokens[&weth].clone(),
            source.common.tokens[&usdc].clone(),
            HashMap::from([
                ("fresh_mm".to_string(), level(3000.0, newest_ms)),
                ("old_mm".to_string(), level(2999.0, newest_ms - 60_000)),
            ]),
            Arc::clone(&source.client),
        );
