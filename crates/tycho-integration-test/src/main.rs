...
use crate::{
    ...
    stream_processor::{
        ...
        rfq_stream_processor::RFQStreamProcessor, StreamUpdate, StreamUpdatePayload,
    },
};
                ...
                match update {
                    Some(update) => {
                        ...
                        match &update.payload {
                            ...
                            StreamUpdatePayload::Rfq { protocol_system, .. } => {
...
async fn process_update(
    ...
    match &update.payload {
        ...
        StreamUpdatePayload::Rfq { protocol_system, received_at, books } => info!(
            "Got {} RFQ update received at {} with {} books",
            protocol_system,
            received_at,
            books.len()
        ),
    ...
    match &update.payload {
        ...
        // RFQ and price level updates carry no sync states.
        StreamUpdatePayload::Rfq { .. } | StreamUpdatePayload::PriceLevelStream(_) => {}
    }
    ...
    // RFQ and price level updates run at a newer block, where an untouched cached state may
    ...
    let oracle_overwrites = match &update.payload {
        ...
        StreamUpdatePayload::Protocol(_) | StreamUpdatePayload::Rfq { .. } => None,
    };
...
fn select_components_to_process(
    ...
    match &update.payload {
        ...
        // RFQ books carry the component next to the state; the processor already sampled them.
        StreamUpdatePayload::Rfq { books, .. } => {
