...
//! routing layer is not built yet (ENG-6293); the cache is built at startup by
//! [`cache::EntityCache::load`] when the mode is not `off`, and every request still reads the
//! database.
//!
